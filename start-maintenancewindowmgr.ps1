...
# =============================================================================
# Theme setup (palette, sidebar, title bar, dialogs: SuiteCommon).
# =============================================================================
...
function Initialize-BgRunspace {
    ...
    $script:BgRunspace = New-SuiteBgRunspace -ModulePath (Join-Path $PSScriptRoot 'Module\MaintWindowMgrCommon.psd1') -LogPath $script:ToolLogPath
}
...
'@
    ...
    $btnAddTarget.Add_Click({
        $picked = Show-CollectionPickerDialog -Owner $window -Collections $script:Collections -Folders $script:Folders -Title 'Add Target Collection' -IncludeBuiltIn $false
        if ($picked) {
...
$btnRemoveWindow.Add_Click({
    ...
    if (-not (Show-ConfirmDialog -Title 'Remove Maintenance Window' -Message ("Permanently delete window '$($row.WindowName)' from $($row.CollectionName)?") -Owner $window)) {
        return
    }
    try {
...
$btnDeleteTemplate.Add_Click({
    ...
    if (-not (Show-ConfirmDialog -Title 'Delete Template' -Message ("Permanently delete template '$($row.Name)' ($([System.IO.Path]::GetFileName($row.FilePath)))?") -Owner $window)) {
        return
    }
    try {
...
$window.Add_Loaded({
    Restore-WindowState -Window $window -Path $global:WindowStatePath -OnStateLoaded {
        param($s)
        if ($s.ActiveView -in @('Windows','Coverage','Templates')) { Set-ActiveView -View ([string]$s.ActiveView) }
    }
