...
### Changed
- **Replaced all WMI queries in `Get-AllMaintenanceWindows` with CM cmdlets** -- eliminates 3 `Get-CimInstance` calls against `SMS_CollectionSettings` and `SMS_Collection`; now uses `Get-CMDeviceCollection` + `Get-CMMaintenanceWindow` per collection; eliminates WS-Management/DCOM connectivity issues

### Removed
- Direct WMI queries against `SMS_CollectionSettings` (bulk discovery + per-collection reload)
- Direct WMI query against `SMS_Collection` for collection name lookup (replaced by `Get-CMDeviceCollection`)
...
- `MaintWindowMgrCommon.psm1` module with 22 exported functions: logging, CM connection, data retrieval (CM cmdlets), CRUD, schedule helpers (including Patch Tuesday calculation), template management, bulk operations, CSV/HTML export
